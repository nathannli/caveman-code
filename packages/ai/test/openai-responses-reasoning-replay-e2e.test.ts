...
describe.skipIf(!process.env.OPENAI_API_KEY || !process.env.ANTHROPIC_API_KEY)(
	...
	() => {
		...
		it("handles same-provider different-model handoff with tool calls", { retry: 2 }, async () => {
			...
			// 1. Model A (gpt-5.4) generates reasoning + function_call
			// 2. User switches to Model B (gpt-5.4-mini) - same provider, different model
			...
			const modelA = getModel("openai", "gpt-5.4");
			const modelB = getModel("openai", "gpt-5.4-mini");
		...
		it("handles cross-provider handoff from Anthropic to OpenAI Codex", { retry: 2 }, async () => {
			...
			const codexModel = getModel("openai", "gpt-5.4-mini");
